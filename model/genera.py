...
def conv_layer(inputs, kernel_size, stride, num_features, idx, nonlinearity=None):
  with tf.variable_scope('Ge_{0}_conv'.format(idx),reuse=tf.AUTO_REUSE) as scope:
...
def transpose_conv_layer(inputs, kernel_size, stride, num_features, idx, nonlinearity=None):
  with tf.variable_scope('Ge_{0}_trans_conv'.format(idx),reuse=tf.AUTO_REUSE) as scope:
    input_channels = int(inputs.get_shape()[3])
...
def fc_layer(inputs, hiddens, idx, nonlinearity=None, flat = False):
  with tf.variable_scope('Ge_{0}_fc'.format(idx),reuse=tf.AUTO_REUSE) as scope:
...
def gen(inputs, nr_res_blocks=1, keep_prob=1.0, nonlinearity_name='concat_elu', gated=True):
